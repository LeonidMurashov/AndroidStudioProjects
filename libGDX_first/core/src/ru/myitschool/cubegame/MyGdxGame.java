...
public class MyGdxGame extends InputAdapter implements ApplicationListener {
    ...
    public void create() {
        ...
        crosshair.setFontScale(3);
        crosshair.setPosition(Gdx.graphics.getWidth() /2 -9, Gdx.graphics.getHeight() / 2 - 27);
    ...
    public void render() {
        ...
        if (Math.abs(pos[1] - startPos[1]) < zone &&
            ...
            isUnder = true;
            crosshair.setColor(Color.VIOLET);
        } else {
            isUnder = false;
    ...
    public boolean touchUp(int screenX, int screenY, int pointer, int button) {
        ...
        if (isUnder && underFire != 0) {
            hits += 1;//System.currentTimeMillis() - underFire;
            underFire = 0;
        } else {
            hits -= 0.7f;
        }
